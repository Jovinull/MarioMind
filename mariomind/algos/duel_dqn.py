...
def train_duel_dqn(run_id: str):
    ...
    q_path = models_path / "mario_q.pth"
    q_target_path = models_path / "mario_q_target.pth"
    score_path = logs_path / "score.p"

    for k in range(1000000):
        s = arrange(_reset_compat(env))
        done = False
        ...
        while not done:
            ...
            if len(memory) > 2000:
                loss += train_step(q, q_target, memory, batch_size, gamma, optimizer, device)
                t += 1

            if t % update_interval == 0:
                copy_weights(q, q_target)
                torch.save(q.state_dict(), q_path)
                torch.save(q_target.state_dict(), q_target_path)
